...
public final class MMOBuffsExpansion extends PlaceholderExpansion {
    ...
    public @Nullable String onRequest(OfflinePlayer player, @NotNull String params) {
        ...
        if (!EffectHolder.has(p)) {
            return null;
        }

        EffectHolder holder = EffectHolder.get(p);
        ...
        switch (option) {
            case "name":
                if (plugin.getEffectManager().has(key)) {
                    return PlainTextComponentSerializer.plainText().serialize(
                            plugin.getEffectManager().get(key).getName()
                    );
                }
                return "";
            case "has":
            ...
            case "duration":
                if (holder.hasEffect(key)) {
                    return holder.getEffect(key).getDurationDisplay();
                }
                return "0";
            case "seconds":
